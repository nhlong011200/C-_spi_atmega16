...
; .FSTART _SPI_Read
; 0000 0021 //	SPDR = 0xFF;
; 0000 0022 	while(!(SPSR & (1<<SPIF)));	/* Wait till reception complete */
_0x3:
	SBIS 0xE,7
	RJMP _0x3
; 0000 0023 	return(SPDR);			/* Return received data */
	IN   R30,0xF
	RET
; 0000 0024 }
; .FEND
...
; 0000 002C      if(data == '1')
	...
	BRNE _0x6
; 0000 002D     {
; 0000 002E         PORTC &= ~0x01;
	CBI  0x15,0
; 0000 002F     }
; 0000 0030     else if(data == '2')
	RJMP _0x7
_0x6:
	CPI  R17,50
	BRNE _0x8
...
; 0000 0032         PORTC |= 0x01;
	SBI  0x15,0
; 0000 0033     }
; 0000 0034     else if(data == '3')
	RJMP _0x9
_0x8:
	CPI  R17,51
	BRNE _0xA
; 0000 0035     {
; 0000 0036         PORTC &= ~0x02;
	CBI  0x15,1
; 0000 0037     }
; 0000 0038     else if(data == '4')
	RJMP _0xB
_0xA:
	CPI  R17,52
	BRNE _0xC
; 0000 0039     {
; 0000 003A         PORTC |= 0x02;
	SBI  0x15,1
; 0000 003B     }
; 0000 003C     else if(data == '5')
	RJMP _0xD
_0xC:
	CPI  R17,53
	BRNE _0xE
; 0000 003D     {
; 0000 003E         PORTC &= ~0x04;
	CBI  0x15,2
; 0000 003F     }
; 0000 0040     else if(data == '6')
	RJMP _0xF
_0xE:
	CPI  R17,54
	BRNE _0x10
...
; 0000 004E DDRA=(1<<DDA7) | (1<<DDA6) | (1<<DDA5) | (1<<DDA4) | (1<<DDA3) | (1<<DDA2) | (1<<DDA1) | (1<<DDA0);
	LDI  R30,LOW(255)
	OUT  0x1A,R30
...
; 0000 0053 // Function: Bit7=In Bit6=Out Bit5=In Bit4=In Bit3=In Bit2=In Bit1=In Bit0=In
; 0000 0054 DDRB=(0<<DDB7) | (1<<DDB6) | (0<<DDB5) | (0<<DDB4) | (0<<DDB3) | (0<<DDB2) | (0<<DDB1) | (0<<DDB0);
	LDI  R30,LOW(64)
	OUT  0x17,R30
; 0000 0055 // State: Bit7=T Bit6=0 Bit5=T Bit4=T Bit3=T Bit2=T Bit1=T Bit0=T
...
; 0000 005A DDRC=(1<<DDC7) | (1<<DDC6) | (1<<DDC5) | (1<<DDC4) | (1<<DDC3) | (1<<DDC2) | (1<<DDC1) | (1<<DDC0);
	LDI  R30,LOW(255)
	OUT  0x14,R30
...
; 0000 00AA // SPI Type: Slave
; 0000 00AB // SPI Clock Rate: 2000.000 kHz
; 0000 00AC // SPI Clock Phase: Cycle Start
; 0000 00AD // SPI Clock Polarity: Low
; 0000 00AE // SPI Data Order: MSB First
; 0000 00AF SPCR=(1<<SPIE) | (1<<SPE) | (0<<DORD) | (0<<MSTR) | (0<<CPOL) | (0<<CPHA) | (0<<SPR1) | (0<<SPR0);
	LDI  R30,LOW(192)
	OUT  0xD,R30
; 0000 00B0 SPSR=(0<<SPI2X);
	LDI  R30,LOW(0)
	OUT  0xE,R30
; 0000 00B1 
; 0000 00B2 // Clear the SPI interrupt flag
; 0000 00B3 #asm
; 0000 00B4     in   r30,spsr
    in   r30,spsr
; 0000 00B5     in   r30,spdr
    in   r30,spdr
; 0000 00B6 #endasm
; 0000 00B7 
